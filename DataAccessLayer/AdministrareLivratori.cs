...
namespace NivelAccesDate
{
    public class AdministrareLivratori : IStocareLivratori
    {
        ...
        public List<Livrator> GetLivatori(int firmaId)
        {
            var result = new List<Livrator>();
            ...
            //var response1 = SqlDBHelper.ExecuteDataSet("CREATE SEQUENCE EMPLOYEE_DG_SEQ START WITH 1 INCREMENT BY 1 CACHE 100", CommandType.Text);
            ...
            //var response7 = SqlDBHelper.ExecuteDataSet("ALTER TABLE EMPLOYEES_DG ADD IS_DELETED NUMBER(1) DEFAULT 1", CommandType.Text);
            //var response8 = SqlDBHelper.ExecuteDataSet("ALTER TABLE DELIVERY_COMPANIES_DG ADD IS_DELETED NUMBER(1) DEFAULT 1", CommandType.Text);



            var dsFirme = SqlDBHelper.ExecuteDataSet("select * from EMPLOYEES_DG where company_id = :IdCompanie AND IS_DELETED = 1", CommandType.Text,
            ...
            {
                result.Add(new Livrator(linieDB));
            }
            return result;
        }

        public Livrator GetLivrator(int id)
        {
            Livrator result = null;
            ...
            if (dsLivratori.Tables[PRIMUL_TABEL].Rows.Count > 0)
            {
                ...
                result = new Livrator(linieDB);
            }
            return result;
        }

        public bool AddLivrator(Livrator livrator)
        {
            
            return SqlDBHelper.ExecuteNonQuery(
                "INSERT INTO EMPLOYEES_DG VALUES (EMPLOYEE_DG_SEQ.nextval, :FirstName, :LastName, :HireDate, :BirthDate, :Email, :CompanyId, :NumberOfDeliveries, :VehicleId, 1)", CommandType.Text,
        ...
        public bool UpdateLivrator(Livrator livrator)
        {
            return SqlDBHelper.ExecuteNonQuery(
                "UPDATE EMPLOYEES_DG set FIRST_NAME = :FirstName, LAST_NAME = :LastName, HIRE_DATE = :HireDate, BIRTH_DATE = :BirthDate, EMAIL = :Email, COMPANY_ID = :CompanyId, NUMBER_OF_DELIVERIES = :NumberOfDeliveries, VEHICLE_ID = :VehicleId, IS_DELETED = 1 where ID = :IdAngajat", CommandType.Text,
                new OracleParameter(":FirstName", OracleDbType.NVarchar2, livrator.FirstName, ParameterDirection.Input),
                new OracleParameter(":LastName", OracleDbType.NVarchar2, livrator.LastName, ParameterDirection.Input),
                new OracleParameter(":HireDate", OracleDbType.Date, livrator.HireDate, ParameterDirection.Input),
                new OracleParameter(":BirthDate", OracleDbType.Date, livrator.BirthDate, ParameterDirection.Input),
                ...
                new OracleParameter(":NumberOfDeliveries", OracleDbType.Int32, livrator.NumberOfDeliveries, ParameterDirection.Input),
                new OracleParameter(":VehicleId", OracleDbType.Int32, livrator.VehicleId, ParameterDirection.Input),
                new OracleParameter(":IdAngajat", OracleDbType.NVarchar2, livrator.Id, ParameterDirection.Input));
        }

        public bool DeleteLivrator(int id)
        {
            return SqlDBHelper.ExecuteNonQuery(
                 "UPDATE EMPLOYEES_DG set IS_DELETED = 0 where ID = :idLivrator", CommandType.Text,
                 new OracleParameter(":idLivrator", OracleDbType.NVarchar2, id, ParameterDirection.Input));
